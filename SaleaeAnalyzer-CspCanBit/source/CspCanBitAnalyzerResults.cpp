...
CspCanBitAnalyzerResults::CspCanBitAnalyzerResults( CspCanBitAnalyzer* analyzer, CspCanBitAnalyzerSettings* settings )
:	AnalyzerResults(),
	mSettings( settings ),
	mAnalyzer( analyzer )
{
...
CspCanBitAnalyzerResults::~CspCanBitAnalyzerResults()
{
}

#define _TEST 0

void CspCanBitAnalyzerResults::GenerateBubbleText( U64 frame_index, Channel& channel, DisplayBase display_base )  //unrefereced vars commented out to remove warnings.
{
	ClearResultStrings();
#if _TEST
	AddResultString("test");
#else
	char number_str[128];
	...
	if (channel == mSettings->mCspCanChannel)
	{
		if ((frame.mType == IdentifierField) || (frame.mType == IdentifierFieldEx))
		{
			itoa((S64)frame.mData1, number_str, 10);

			AddResultString("ORB: ", number_str);
			//AddResultString("ORN");
		}
		else if (frame.mType == CalibrationField)
		{
			itoa((S64)frame.mData1, number_str, 10);

			AddResultString("Calib: ", number_str);
		}
		else if (frame.mType == DataField)
		{
			itoa((S64)frame.mData1, number_str, 10);
		...
		else
		{
			...
			//AnalyzerHelpers::GetNumberString( bitDiff, display_base, 16, number_str, 128 );
			AddResultString("ERROR TYPE");
		}
	}
	else
	{
...
void CspCanBitAnalyzerResults::GenerateExportFile( const char* file, DisplayBase display_base, U32 /*export_type_user_id*/ )
{
...
void CspCanBitAnalyzerResults::GenerateFrameTabularText( U64 frame_index, DisplayBase display_base )
{
    ClearTabularText();
	char number_str[128];
...
void CspCanBitAnalyzerResults::GeneratePacketTabularText( U64 /*packet_id*/, DisplayBase /*display_base*/ )  //unrefereced vars commented out to remove warnings.
{
	ClearResultStrings();
	AddResultString( "not supported" );
}

void CspCanBitAnalyzerResults::GenerateTransactionTabularText( U64 /*transaction_id*/, DisplayBase /*display_base*/ )  //unrefereced vars commented out to remove warnings.
{
	ClearResultStrings();
	AddResultString( "not supported" );
}
