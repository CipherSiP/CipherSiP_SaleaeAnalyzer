...
CspSpiAnalyzer::CspSpiAnalyzer()
:	Analyzer2(),
	mSettings( new CspSpiAnalyzerSettings() ),
...
void CspSpiAnalyzer::SetupResults()
{
	mResults.reset( new CspSpiAnalyzerResults( this, mSettings.get() ) );
...
void CspSpiAnalyzer::WorkerThread()
{
	Setup();
...
void CspSpiAnalyzer::AdvanceToActiveEnableEdgeWithCorrectClockPolarity()
{
...
void CspSpiAnalyzer::AdvanceToActiveEnableEdge()
{
	if( mEnable != NULL )
	{
...
bool CspSpiAnalyzer::IsInitialClockPolarityCorrect()
{
...
bool CspSpiAnalyzer::WouldAdvancingTheClockToggleEnable()
{
	if( mEnable == NULL )
		return false;
...
void CspSpiAnalyzer::GetWord()
{
	...
	if (mFrameNum == 0) // Register type
	{
		result_frame.mType = FRAME_HEADER;
	}
	else
	{
		result_frame.mType = FRAME_DATA;
	}
	result_frame.mData1 = SET_FRAME_DATA(mFrameNum, mosi_word);
	result_frame.mData2 = SET_FRAME_DATA(mFrameNum, miso_word);
	mFrameNum++;
...
bool CspSpiAnalyzer::NeedsRerun()
{
	return false;
}

U32 CspSpiAnalyzer::GenerateSimulationData( U64 minimum_sample_index, U32 device_sample_rate, SimulationChannelDescriptor** simulation_channels )
{
...
U32 CspSpiAnalyzer::GetMinimumSampleRateHz()
{
	return 10000; //we don't have any idea, depends on the SPI rate, etc.; return the lowest rate.
}

const char* CspSpiAnalyzer::GetAnalyzerName() const
{
	return "CSP_SPI";
}
