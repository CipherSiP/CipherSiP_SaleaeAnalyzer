...
CspSpiAnalyzerSettings::CspSpiAnalyzerSettings()
...
bool CspSpiAnalyzerSettings::SetSettingsFromInterfaces()
{
...
void CspSpiAnalyzerSettings::LoadSettings( const char* settings )
{
	...
	if( strcmp( name_string, "SaleaeCspSpiAnalyzer" ) != 0 )
		AnalyzerHelpers::Assert( "SaleaeCspSpiAnalyzer: Provided with a settings string that doesn't belong to us;" );
...
const char* CspSpiAnalyzerSettings::SaveSettings()
{
	SimpleArchive text_archive;

	text_archive << "SaleaeCspSpiAnalyzer";
	text_archive <<  mMosiChannel;
	text_archive <<  mMisoChannel;
	text_archive <<  mClockChannel;
	text_archive <<  mEnableChannel;
...
void CspSpiAnalyzerSettings::UpdateInterfacesFromSettings()
{
	mMosiChannelInterface->SetChannel( mMosiChannel );
	mMisoChannelInterface->SetChannel( mMisoChannel );
	mClockChannelInterface->SetChannel( mClockChannel );
